...
const noteClass = 'rounded-2xl border border-blue-400/20 bg-blue-400/10 p-3 text-xs leading-5 text-blue-100 md:text-sm';
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-200">Cotizacion estimada</p>
          <h3 className="mt-1 text-xl font-black">Solicita tu presupuesto</h3>
        </div>
        <span className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-[var(--muted)]">
          {step + 1}/{steps.length}
        </span>
      </div>
      ...
      <div className="mt-4 min-h-[430px] animate-fade-up">
        {step === 0 && (
          <div>
              ...
              {serviceTypes.map((serviceType) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'group min-h-[150px] rounded-2xl border p-4 text-left transition duration-200 hover:-translate-y-0.5 md:p-5',
                      isSelected
                  ...
                  >
                    ...
                    <span className="mt-4 flex items-start justify-between gap-3">
                      <span>
                        <span className="block text-lg font-black text-white">{serviceType.title}</span>
                        <span className="mt-1.5 block text-sm leading-6 text-[var(--muted)]">{serviceType.description}</span>
                      </span>
        ...
        {step === 2 && (
          <div>
            <p className="text-sm leading-6 text-[var(--muted)]">Elige el espacio donde necesitas la instalacion.</p>
              ...
              {installationTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'rounded-2xl border p-4 text-left text-sm font-black transition duration-200 hover:-translate-y-0.5',
                      isSelected
        ...
        {step === 4 && (
          <div>
            <p className="text-sm leading-6 text-[var(--muted)]">Marca solo lo que aplique. Esto ayuda a estimar mejor el trabajo.</p>
        ...
        {step === 5 && (
          <div>
            ...
            <label
              ...
              onDrop={onDrop}
              className={cn(
                'mt-3 flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed p-6 text-center transition',
                isDragging ? 'border-blue-300 bg-blue-400/15' : 'border-white/20 bg-white/[0.04] hover:border-blue-300/50 hover:bg-white/[0.07]'
              )}
            >
              <UploadCloud className="text-blue-200" size={28} />
            ...
            {photos.length > 0 ? (
              <div className="mt-3 grid grid-cols-3 gap-2.5">
                {photos.map((photo) => (
                  <div key={photo.url} className="group relative aspect-square overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={photo.url} alt={photo.file.name} className="h-full w-full object-cover" />
                    <button
                      type="button"
                      onClick={() => removePhoto(photo.url)}
                      className="absolute right-1.5 top-1.5 grid h-8 w-8 place-items-center rounded-full bg-black/65 text-white transition hover:bg-red-500"
              ...
              <div className="mt-3 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-sm text-[var(--muted)]">
        ...
        {step === 7 && (
          <div>
            ...
            {sent && (
              <p className="mt-3 rounded-2xl border border-green-300/20 bg-green-400/10 p-3 text-sm text-green-100">
